    ...
    <section class="content">
      ...
      <div class="box box-primary">
        <div class="box-header with-border">
          <h3 class="box-title">Running: {{.running}} / {{.total}} </h3>
          <div class="pull-right"><a href="#" {{.full1}}><input type="checkbox" class="minimal" {{.full2}}> Show full containers</a></div>
        </div>
        <table id="containerTable" class="table table-bordered table-hover">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{html .status}}</td>
              <td><a href="/farm/container/stat?id={{.id}}">{{.id}}</a></td>
              <td><a href="/farm/container/stat?id={{.id}}">{{html .name}}</a></td>
              <td>{{.ip}}</td>
              <td>{{html .hostname}}</td>
              <td>{{html .image}}</td>
              <td>{{html .command}}</td>
              <td>
                <div class="btn-group">
                  {{if eq .state "running"}}
                    <button value="{{.id}}" type="button" class="stopBtn btn btn-default"><i class="fa fa-stop"></i></button>
                  {{else}}
                    <button value="{{.id}}" type="button" class="startBtn btn btn-default"><i class="fa fa-play"></i></button>
                  {{end}}
                  <button value="{{.id}}" type="button" class="restartBtn btn btn-default"><i class="fa fa-refresh"></i></button>
                </div>
              </td>
            </tr>
            {{end}}
...
$(function () {
  ...
  $(".startBtn").click(function () {
    ...
    $.ajax({
      method: "GET",
      url: "/farm/container/control?do=start&id=" + id,
      success: function (data) {
        ...
        document.location="/farm/container";
      },
  ...
  $(".restartBtn").click(function () {
    ...
    $.ajax({
      method: "GET",
      url: "/farm/container/control?do=restart&id=" + id,
      success: function (data) {
        if (data != "<nil>")
          alert(data);
        else {
  ...
  $(".stopBtn").click(function () {
    ...
    $.ajax({
      method: "GET",
      url: "/farm/container/control?do=stop&id=" + id,
      success: function (data) {
        if (data != "<nil>")
          alert(data);
        else {
