      ...
      <ul class="sidebar-menu">
        ...
        <li class="treeview active">
          ...
          <ul class="treeview-menu">
            ...
            <li><a href="/farm/container"><i class="fa fa-circle"></i> Containers</a></li>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active"><a href="/farm/container">Container</a></li>
      </ol>
    </section>
        ...
        <div class="col-md-3">
          ...
          <div class="box">
            <div class="box-header with-border">
              <h3 class="box-title">{{html .name}}</h3>
              <span class="pull-right">
                {{if eq .state "running"}}
                  <span class="label bg-green"><i class="fa fa-play margin-r-5"></i>{{html .state}}</span>
                {{else}}
                  <span class="label bg-red"><i class="fa fa-stop margin-r-5"></i>{{html .state}}</span>
                {{end}}
              </span>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <strong><i class="fa fa-circle-o-notch margin-r-5"></i> Host:</strong>
              <p class="text-muted" id="upTime">{{html .host}}</p>
              <hr>

              <strong><i class="fa fa-clock-o margin-r-5"></i> PID:</strong>
              <p class="text-muted" id="curTime" style="word-wrap: break-word">{{.pid}}</p>
              <hr>

              <strong><i class="fa fa-cloud margin-r-5"></i> Image:</strong>
              <p class="text-muted">{{html .image}}</p>
              <hr>

              <strong><i class="fa fa-gear margin-r-5"></i> Hostname:</strong>
              <p class="text-muted">{{html .hostname}}</p>
              <hr>
        ...
        <div class="col-md-9">
          ...
          <div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
              <li class="active"><a href="#processTab" data-toggle="tab">Network</a></li>
              <li><a href="#containerTab" data-toggle="tab">Port Map</a></li>
              <li><a href="#mountTab" data-toggle="tab">Command</a></li>
              <li><a href="#networkTab" data-toggle="tab">Volumne</a></li>
            </ul>
            <div class="tab-content">

              <div class="active tab-pane" id="processTab" style="text-align: center">
                <img src="/static/dist/img/coming-soon-teddy-sample.jpg">
              </div>

              <div class="tab-pane" id="containerTab" style="text-align: center">
                <img src="/static/dist/img/coming-soon-teddy-sample.jpg">
              </div>
              ...
              <div class="tab-pane" id="networkTab" style="text-align: center">
                <img src="/static/dist/img/coming-soon-teddy-sample.jpg">
              </div>
...
$(function () {
  ...
  function updateChart(c, y) {
    for (i=0; i<y.length; ++i) {
      c.data.datasets[i].data.splice(0,1);
      c.data.datasets[i].data.push(y[i]);
    }
    c.update();
  }
  ...
  function update() {
    $.ajax({
      method: "GET",
      url: "/farm/container/livestat?id={{printf "%.12s" .id}}",
      success: function (data) {
        updateChart(cpuChart, [(data["cpu"]-oldCPU)/2]);
        oldCPU = data["cpu"]
        updateChart(memoryChart, [data["mem"]]);
        var tx = 0;
        var rx = 0;
        for (var key in data["net"]) {
          tx += data["net"][key]["tx_bytes"];
          rx += data["net"][key]["rx_bytes"];
        }
        tx /= 1024;
        rx /= 1024;
        updateChart(networkChart, [(tx-oldTX)/2, (rx-oldRX)/2]);
        oldTX = tx; oldRX = rx;
      ...
      timeout: function (data) {
        ...
        updateChart(memoryChart, [0]);
        updateChart(networkChart, [0, 0]);
        updateChart(storageChart, [0, 0]);
      }
    });
    if (hostOnline) setTimeout(update, 5000);
  }
  update();
